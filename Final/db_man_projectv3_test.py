...
def load_employee_is_active():  #load  id,name,surname of employees
    connection=connect()
    try:
...
def remove_item_from_selected_employee(assign_id):
    ...
    try:
        with connection.cursor() as cursor:
            quantity_query = "select quantity,item_id from employee_items where assign_id=%s"
            cursor.execute(quantity_query, (assign_id,))
            result = cursor.fetchone()
            print(result)
            if result is None:
...
def see_events(): ##^^##
    try:
        ...
        with connection.cursor() as cursor:
            ...
            query="""select  * from events_ where event_date >=NOW(); """
            cursor.execute(query)
            events=cursor.fetchall()
...
def create_special_request(employee_id, request_type,description,request_amount=None):#Çalışan id'si ile talep oluşturur
...
def create_leave_request(employee_id, leave_type, start_date, end_date,desc_request):#Çalışan id'si ile izin oluşturur
    print(employee_id, leave_type, start_date, end_date)
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            (employee_id, request_date, leave_type, Start_date, end_date, total_dates,created_at,desc_request) 
...
def add_daily_working_hours(employee_id, work_date, attendance, clock_in_time, 
    ...
    date1 = datetime.datetime.strptime(clock_in_time, date_format)
    date2 = datetime.datetime.strptime(clock_out_time, date_format)
