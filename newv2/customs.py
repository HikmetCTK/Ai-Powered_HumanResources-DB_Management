...
class CustomQWidget(QWidget):
    clickedValue = pyqtSignal(int)  # special signal, int will be returned

    def __init__(self, parent):
        super().__init__(parent=parent)

    def mousePressEvent(self, event):
        if self.objectName() == "widget_summary_1":
            value = 10001
        elif self.objectName() == "widget_summary_2":
            value = 10002
        elif self.objectName() == "widget_summary_3":
            value = 10003
        elif self.objectName() == "widget_summary_4":
            value = 10004
        elif self.objectName() == "widget_summary_5":
            value = 10005
        elif self.objectName() == "widget_summary_6":
            value = 10006
        else:
            value = 10007
...
class MonoButtonWidget(QWidget):
    ...
    def __init__(self, row_index, row_data, DynamicProperties:dict):
        super().__init__()
        ...
        if self.buttonType.casefold() == "read":
            self.iconPath = getPath("view")
        elif self.buttonType.casefold() == "add":
            self.iconPath = getPath("add")
        elif self.buttonType.casefold() == "select":
            self.iconPath = getPath("select_1")
        elif self.buttonType.casefold() == "delete":
            self.iconPath = getPath("cross_mark_3")
        else:
        ...
        self.dynamicMonoButton.setIcon(QIcon(self.iconPath))
    ...
    def monoButtonClicked(self):
        ...
        self.clicked_value.emit((self.rowDataMatch[objectID][0], self.buttonType))
...
class DoubleButtonWidget(QWidget):
    ...
    def __init__(self, row_index, row_data, DynamicProperties:None|dict = None):
        ...
        self.dynamicAcceptButton.setIcon(QIcon(getPath("accept")))
        ...
        self.dynamicRejectButton.setIcon(QIcon(getPath("decline")))
...
class PeopleCardWidget():
    def createPeopleCardWidget(obj):
        ...
        obj.lbl_people_card_profile_image.setPixmap(QPixmap(u"C:/Users/Fevzi/Downloads/profile_3.png"))
    ...
    def getPeopleCard(obj, name):
        ...
        liste = db_man_projectv3.load_employee_for_message_selection(name)
        ...
        for i in range(len(liste)):
            ...
            widget_action.triggered.connect(obj.handleMessagePeopleCardClick)
...
def setTable(obj, rowCount:int, columnCount:int, table:QTableWidget, items:list, rowHeaders:list = None, columnHeaders:list = None,
             cellWidgetAppend:bool = False, ButtonWidgetType:None|str = None, DynamicProperties:None|dict = None):

    # table.setUpdatesEnabled(False)
    # table.blockSignals(True)
    table.setSortingEnabled(False)
    _setTableProperties(obj, rowCount, columnCount, table, rowHeaders, columnHeaders)
    _setTableContent(obj, rowCount, columnCount, table, items, cellWidgetAppend, ButtonWidgetType, DynamicProperties)
...
def _createControls(obj, pairs:dict, buttons:list|None = None, buttonColumnCount:int = 2, rowSpan:int = 1, colSpan:int = 1):
    ...
    for header, content in pairs.items():
        ...
        for item in obj.nameDisabledLineEdits:
            if db_man_projectv3.arrangeText(item) in db_man_projectv3.arrangeText(header):
                obj.dynamic_lineEdit.setDisabled(True)

        obj.verticalLayout_17.addWidget(custom_widget)
...
def createSideMenuWidgets(self, row_contents_dict, desiredDynamicButtons, desiredDynamicButtonContainerColumnSpan = 2, rowSpan = 1, colSpan = 1):
    _createControls(self, row_contents_dict, desiredDynamicButtons, desiredDynamicButtonContainerColumnSpan, rowSpan, colSpan)
